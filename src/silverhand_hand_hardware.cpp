#include "silverhand_hand_control/silverhand_hand_hardware.hpp"
...
#include "silverhand_hand_control/joint_ports.hpp"
...
TYPE_ALIAS(JointStateMsg, reg_udral_physics_kinematics_translation_Linear_0_1)
  ...
  const std::uint16_t default_value)
{
  ...
  try {
    ...
    RCLCPP_WARN(
      rclcpp::get_logger("silverhand_hand_control"),
...
namespace silverhand_hand_control::detail
{
...
public:
  ...
  : AbstractSubscription<JointStateMsg>(interface, silverhand_hand_control::kAgentJointStatePort),
    runtime_(runtime)
  {
  }

  void handler(
    const reg_udral_physics_kinematics_translation_Linear_0_1 & message,
    CanardRxTransfer * transfer) override
  {
    ...
    if (remote_node_id < kFirstGripperNodeId ||
      remote_node_id >= (kFirstGripperNodeId + kJointCount))
    {
      return;
    }

    const auto index = static_cast<std::size_t>(remote_node_id - kFirstGripperNodeId);
    runtime_->joint_position[index] = message.position.meter;
    runtime_->joint_velocity[index] = message.velocity.meter_per_second;
  }
...
SilverhandHandSystem::~SilverhandHandSystem() = default;

CallbackReturn SilverhandHandSystem::on_init(
  const hardware_interface::HardwareInfo & hardware_info)
{
  ...
  if (info_.joints.size() != kJointCount) {
    RCLCPP_ERROR(
      get_logger(),
      "Expected %zu joints for SilverhandHandSystem, got %zu",
  ...
  joint_effort_state_.assign(joint_count, 0.0);
...
CallbackReturn SilverhandHandSystem::start_transport()
{
  stop_transport();
...
void SilverhandHandSystem::stop_transport()
{
  if (runtime_) {
...
CallbackReturn SilverhandHandSystem::on_configure(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  return start_transport();
}

CallbackReturn SilverhandHandSystem::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
...
CallbackReturn SilverhandHandSystem::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  is_active_ = false;
  return CallbackReturn::SUCCESS;
...
CallbackReturn SilverhandHandSystem::on_cleanup(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  is_active_ = false;
  stop_transport();
...
std::vector<hardware_interface::StateInterface> SilverhandHandSystem::export_state_interfaces()
{
...
std::vector<hardware_interface::CommandInterface> SilverhandHandSystem::export_command_interfaces()
{
...
void SilverhandHandSystem::spin_transport_once()
{
  if (runtime_ && runtime_->interface) {
...
void SilverhandHandSystem::send_heartbeat()
{
  if (!runtime_ || !runtime_->interface) {
    return;
  }
...
void SilverhandHandSystem::send_joint_command(
  const std::size_t joint_index, const float position, const float effort)
{
  ...
  reg_udral_physics_kinematics_translation_Linear_0_1 command = {
    .position = {position},
    // The current gripper Cyphal message carries position/velocity/acceleration only.
    // Effort is exported via ros2_control but not transmitted until the wire protocol is extended.
    .velocity = {0.0F},
    .acceleration = {0.0F}
  };

  joint_effort_state_[joint_index] = effort;
...
hardware_interface::return_type SilverhandHandSystem::read(
...
hardware_interface::return_type SilverhandHandSystem::write(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
...
PLUGINLIB_EXPORT_CLASS(
  silverhand_hand_control::SilverhandHandSystem,
